...
class GestureRecognizer:
    ...
    def main(self):
        ...
        print("Make Gesture then Press Enter to continue: ")
        ...
        self.asl_text = list(map(lambda x: x.replace('\r', '').replace("_"," "), self.asl_text))
        # [print(char, end="") for char in self.asl_text]

        string = ""
        for i in self.asl_text:
            string += i
    ...
    def asl_spell(self):
        '''
        Writes the letters of the detected gestures to the console.
        '''
        self.lock.acquire()
        gestures = self.current_gestures
        self.lock.release()
        if cv2.waitKey(1) & 0xFF == 13:
            for hand_gesture_name in gestures:          
                ...
                self.asl_text.append(hand_gesture_name)
